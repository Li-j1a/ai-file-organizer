...
async function smartSearch(query) {
  ...
  return fileIndex.filter((file) => {
    return file.name.toLowerCase().includes(lowerQuery) || file.path.toLowerCase().includes(lowerQuery);
  }).slice(0, 100);
}
async function aiSemanticSearch(searchParams) {
  ...
  if (filters.extensions && filters.extensions.length > 0) {
    ...
    filteredFiles = filteredFiles.filter(
      (file) => allowedExts.some((ext) => file.ext.toLowerCase() === ext.toLowerCase())
    );
  }
  ...
  for (const file of filteredFiles) {
    try {
      ...
      const contentLength = filters.contentLength || 2e3;
  ...
  return results.filter((r) => r.relevance >= 20).sort((a, b) => b.relevance - a.relevance);
}
...
async function getProcessList() {
  ...
  return processes.list.sort((a, b) => b.memRss - a.memRss).slice(0, 50).map((p) => ({
    pid: p.pid,
    name: p.name,
    cpu: p.cpu,
    memory: p.memRss
...
async function suggestFileName(filePath) {
  ...
  try {
    ...
    if (textExtensions.includes(ext) && stats.size < 200 * 1024) {
      ...
      fileContent = extracted.getBody().substring(0, 2e3);
      ...
      fileContent = csvContent.substring(0, 2e3);
...
直接输出文件名：`;
  ...
  const result = await callAI(prompt);
  let cleanName = result.trim().replace(/^["'「『]|["'」』]$/g, "").replace(/^建议的?文件名[：:]\s*/i, "").replace(/^新?文件名[：:]\s*/i, "").replace(/^文件名[：:]\s*/i, "").split("\n")[0].trim();
...
function getTimeCategory(modifiedDate) {
  ...
  const diffDays = Math.floor((now - fileDate) / (1e3 * 60 * 60 * 24));
  ...
  for (const file of files) {
    ...
    if (useAI) {
      try {
        ...
        if (textExtensions.includes(file.ext) && stats.size < 200 * 1024) {
          fileContent = await fs.readFile(file.path, "utf-8");
          fileContent = fileContent.substring(0, 2e3);
        } else if (file.ext === ".docx" && stats.size < 5 * 1024 * 1024) {
          const buffer = await fs.readFile(file.path);
          const result = await mammoth.extractRawText({ buffer });
          fileContent = result.value.substring(0, 2e3);
        } else if (file.ext === ".doc" && stats.size < 5 * 1024 * 1024) {
          const extracted = await wordExtractor.extract(file.path);
          fileContent = extracted.getBody().substring(0, 2e3);
        } else if ([".xlsx", ".xls"].includes(file.ext) && stats.size < 5 * 1024 * 1024) {
          ...
          const sheetName = workbook.SheetNames[0];
          const sheet = workbook.Sheets[sheetName];
          fileContent = XLSX.utils.sheet_to_csv(sheet).substring(0, 2e3);
        }
        if (fileContent) {
